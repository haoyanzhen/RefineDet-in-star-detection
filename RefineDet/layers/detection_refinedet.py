...
from layers.box_utils import decode, nms, center_size
...
class Detect_RefineDet(Function):
    ...
    def forward(self, arm_loc_data, arm_conf_data, odm_loc_data, odm_conf_data, prior_data):
        ...
        no_object_index = arm_object_conf <= self.objectness_thre  # 阈值0
        conf_data[no_object_index.expand_as(conf_data)] = 0

        num = loc_data.size(0)  # batch size
        num_priors = prior_data.size(0)  # prior box num
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):
                c_mask = conf_scores[cl].gt(self.conf_thresh)  # 阈值1
                ...
                #print(boxes, scores)
                ids, count = nms(boxes, scores, self.nms_thresh, self.top_k)  # 阈值2，3 反重叠
                output[i, cl, :count] = \
        ...
        _, rank = idx.sort(1)
        flt[(rank < self.keep_top_k).unsqueeze(-1).expand_as(flt)].fill_(0)  # 阈值4
        return output
